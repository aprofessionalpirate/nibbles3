...
SUB GetInputs (NumPlayers, speed, diff$, monitor$)
    ...
    LOOP UNTIL VAL(num$) = 1 OR VAL(num$) = 2 OR VAL(num$) = 3
    NumPlayers = VAL(num$)
...
SUB Level (WhatToDO, sammy() AS snaketype) STATIC
    ...
    CASE 1
        sammy(1).row = 25: sammy(2).row = 25: sammy(3).row = 15
        sammy(1).col = 50: sammy(2).col = 30: sammy(3).col = 50
        sammy(1).direction = 4: sammy(2).direction = 3: sammy(3).direction = 1
    ...
    CASE 3
        ...
        sammy(1).row = 25: sammy(2).row = 25: sammy(3).row = 15
        sammy(1).col = 50: sammy(2).col = 30: sammy(3).col = 50
        sammy(1).direction = 1: sammy(2).direction = 2: sammy(3).direction = 3
    ...
    CASE 6
        ...
        NEXT i
        sammy(1).row = 7: sammy(2).row = 43: sammy(3).row = 20
        sammy(1).col = 65: sammy(2).col = 15: sammy(3).col = 60
        sammy(1).direction = 2: sammy(2).direction = 1: sammy(3).direction = 3
    ...
    CASE 9
        ...
        NEXT i
        sammy(1).row = 40: sammy(2).row = 15: sammy(3).row = 40
        sammy(1).col = 75: sammy(2).col = 5: sammy(3).col = 40
        sammy(1).direction = 1: sammy(2).direction = 2: sammy(3).direction = 1
    ...
    DO
        ...
        DO
            ...
            SELECT CASE kbd$
                ...
                CASE "u", "U": IF sammy(3).direction <> 2 THEN sammy(3).direction = 1
                CASE "j", "J": IF sammy(3).direction <> 1 THEN sammy(3).direction = 2
                CASE "h", "H": IF sammy(3).direction <> 4 THEN sammy(3).direction = 3
                CASE "k", "K": IF sammy(3).direction <> 3 THEN sammy(3).direction = 4
            ...
            FOR a = 1 TO NumPlayers
                ...
                IF PointIsThere(sammy(a).row, sammy(a).col, colorTable(4)) OR (sammy(1).row = sammy(2).row AND sammy(1).col = sammy(2).col) OR (sammy(1).row = sammy(3).row AND sammy(1).col = sammy(3).col) OR (sammy(2).row = sammy(3).row AND sammy(2).col = sammy(3).col) THEN
                    PLAY "MBO0L32EFGEFDC"
                    COLOR , colorTable(4)
                    LOCATE numberRow, NumberCol
                    PRINT " "
        ...
        FOR a = 1 TO NumPlayers
            ...
            IF sammy(a).alive = FALSE THEN
                ...
                ELSEIF a = 2 THEN
                    SpacePause " <---- Luke Dies! Push Space "
                ELSE
                    SpacePause " <---- David Dies! Push Space "
                END IF
            END IF
        NEXT a
...
SUB PrintScore (NumPlayers, score1, score2, score3, lives1, lives2, lives3)
    ...
    IF NumPlayers = 2 THEN
        LOCATE 1, 1
        PRINT USING "#,###,#00  Lives: #  <--LUKE"; score2; lives2
	END IF
	...
	IF NumPlayers = 3 THEN
		...
		LOCATE 2, 1
        PRINT USING "#,###,#00  Lives: #  <--DAVID"; score3; lives3
    END IF
